import {
  ...
  DialogTitle, IconButton, LinearProgress, MenuItem, Paper,
  Slider, Table, TableBody, TableCell, TableContainer, TableHead,
  TableRow, TextField, Tooltip, Typography
} from '@mui/material'
import { Add, Delete, Edit, ExpandMore, LockOutlined } from '@mui/icons-material'
...
export default function PlansPage() {
  ...
  const savePlan = () => {
    if (!planForm.employee_id || !planForm.title) { notify('Employee and title required', 'warning'); return }
    setSaving(true)
    setTimeout(() => {
      if (editingPlan) {
        ...
        setPlans((prev) => [...prev, { ...planForm, id: `p${Date.now()}`, goals: [], created_at: new Date().toISOString(), updated_at: new Date().toISOString() }])
        notify('Plan created', 'success')
      }
      setSaving(false); setPlanOpen(false)
    }, 400)
  }
    ...
    setTimeout(() => {
      if (editingGoal) {
        setPlans((prev) => prev.map((p) => ({
          ...p,
          goals: p.goals.map((g) => g.id === editingGoal.id ? { ...g, ...goalForm } : g)
        })))
        notify('Goal updated', 'success')
      } else {
        const newGoal = { ...goalForm, id: `g${Date.now()}`, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }
        setPlans((prev) => prev.map((p) => p.id === goalForm.plan_id ? { ...p, goals: [...(p.goals || []), newGoal] } : p))
        notify('Goal created', 'success')
      }
      setSaving(false); setGoalOpen(false)
    }, 400)
  }
